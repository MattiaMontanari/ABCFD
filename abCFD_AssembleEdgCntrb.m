...
function [ Contribut ] = abCFD_AssembleEdgCntrb( ShapInt, DofTag, ...
                            DofNum, DofCoord  )

%% Initialize
% Obtain number of elements and connectivity for elements on \Gamma
[NumEle , NumLocalDof] = size( DofTag ) ;

%% Loop over each element
% Initialized output
Contribut = sparse( DofNum , 1+( (ShapInt-1)*(DofNum-1) ) );
for id_ele = 1 : NumEle
    ...
    [ Q ] = abCFD_GPquad( NumLocalDof , a, b, ShapInt );
...
%         a formula which gives: 1 if ShapInt=1 or DofNum if ShapInt=2
