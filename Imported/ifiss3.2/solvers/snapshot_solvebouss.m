...
n_null = null_pressure_index(domain,qmethod,size(Bbc,1));
   ...
   elseif precon==9,  %%%% new PCD   
       if exist('map','var')==0,
       [dddx,dddy,dddxy,dddxyp,dddmp,map] = q2q1gridx(x,y,xy2,mv2,boundv); end
...
if precon_format==1, 
   ...
   % in preconditioner
      ...
      elseif precon==9 ,  %%%%  modified pressure commutator (newPCD) 
         ...
         mfun_par = struct('Mfun','m_bouss_xfp','Fp',Fp,'G',Qv,'Mp',Qp,...
                             'domain',domain, 'n_null',n_null);
      else     %%%% precon==3, LSC
   ...
   %  compute new MG data or reload existing data?
    ...
    if damps==1
     ...
     elseif precon==3, %%% AMG iterated LSC 
         ...
         else, amg_gridA = amg_grids_setup(Bbc*(Gdiag\Bbc')); end
