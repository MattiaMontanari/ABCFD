...
%
   ...
   % in preconditioner
      ...
      elseif precon==4 ,  %%%%  modified pressure commutator (Fp) 
	     ...
	     if qmethod>1,
		    mfun_par = struct('Mfun','m_xfp','Fp',Fp,'G',G,'Mp',Q,'domain',domain,'n_null',n_null);
			elseif qmethod==0,
			[Cp1,Cp2] = Cpre_q1q1(xy,ev);
			mfun_par = struct('Mfun','m_sxfp','Fp',Fp,'G',G,'Mp',Q,'domain',domain,...
								   'Cp1',Cp1,'n_null',n_null,'viscosity',viscosity);
			elseif qmethod==1,
...
%
 ...
 %    compute new MG data or reload existing data?
	...
	Gdiag=spdiags(diag(G),0,nnv,nnv);
		...
		error('AMG iterated PCD* preconditioner is not yet implemented for this approximation!') 		 
		end
    amg_gridA = amg_grids_setup(Apx);
