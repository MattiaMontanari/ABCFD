...
f = viscosity*a + f;
%%
%%% fix inflow pressures for the systems with inflow boundary
xmin = min(xy(:,1));
...
%   nu p_x = w1 p For Fp at inflow
    ...
    for edge=1:length(uxbd),
       f(boundp(edge),boundp(edge)) = f(boundp(edge),boundp(edge)) + uxbd(edge)*hy(boundp(edge));
    end
else
