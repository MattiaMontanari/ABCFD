function [a,f] = fpszsetup_q1(xy,xyp,mv,mp,flowsol,viscosity,domain,map,cp1)
%FPSZSETUP_Q1 modified Q1 PCD matrix with stabilization 
%   [Ap,Fp] = fpszsetup_q1(xy,xyp,mv,mp,flowsol,viscosity,domain,map);
%   input
...
%          cp1        Q1 stabilization matrix
%   output
%          Ap         Q1 scalar diffusion matrix with stabilization
%          Fp         Q1 scalar convection-diffusion matrix with
%                     stabilization
%
...
% find pressure and velocity indices
   boundp=find(xyp(:,1)==xmin);
   boundv=map(boundp);
   nbd=length(boundp); 
   uxbd=usol(boundv);
%fprintf('check inflow boundary velocity\n')
%disp([xyp(boundp,1), xyp(boundp,2), uxbd])
