function [xjmp,yjmp,els] = stressjmps_q2p1(viscosity,q2p1sol,eex,...
                                                 xy,mv,mbound,neumannb,s)
%STRESSJMPS_Q2P1 vectorised stress jumps for Q2-P1 grid 
%   [xjmp,yjmp]=stressjmps_q2p1(viscosity,q2p1sol,eex,...
%                                           xy,mv,mbound,neumannb,s);
%   input
...
% Copyright (c) 2010 D.J. Silvester, Qifeng Liao 
	  ...
	  usol=q2p1sol(1:nvtx); vsol=q2p1sol(nvtx+1:2*nvtx); 
...
% nx= 0, ny=-1    
    ...
    [chi,dchidx,dchidy] = lderiv(s,-1,xl_v,yl_v);
...
% nx= 1, ny= 0
    ...
    [chi,dchidx,dchidy] = lderiv(1,s,xl_v,yl_v);
    xfx_v=zero_v; yfx_v=zero_v;
    psol=zero_v;
    for  ivtx=1:9 		 
