...
%   box_domain;
%  Calculates Q2 element partitioning information for the domain [0,1]x[0,H] 
%  with Dirichlet data on the vertical boundary edges. 
% 
%  Grid defining data is saved to  box_grid
%   IFISS function: DJS; 4 May 2012.
% modification of code written by M.D. Mihajlovic.
% Copyright (c) 2009 D.J. Silvester, H.C. Elman, A. Ramage 

global pde
fprintf('\nGRID GENERATION FOR [0,1]x[0,H] DOMAIN: \n');
L=1;
H=default('Input the domain height H (default 2)',2);
gt=default('Grid type: uniform/stretched 1/2 (default uniform)', 1);
gh=1;  %%single grid
      if(H>=L)   %  vertical cavity
         npx=default('Input the number of elements in x direction (default 16)',16);
         if(npx<=0)
            error('Illegal choice, try again.');
         end
      else       %  horizontal cavity
         npy=default('Input the number of elements in y direction (default 16)',16);
         if(npy<=0)
            error('Illegal choice, try again.');
         end
      end   
...
hty=2; %% side heating enforced here  
fprintf('Type of BC for the temperature on horizontal boundaries:');    
...
save box_grid.mat mv xy bound mbound H L nh grid_type x y hty
clear
return
