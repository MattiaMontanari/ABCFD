% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %%
%    /\   |     __  ___  __                                                    %
%   /  \  |__  /   |    |  \  	COMPUTATION OF THE REDUCED ORDER MODEL FOR A   %
%  /----\ |  \|    |--  |   |   RANGE OF MODES SPECIFIED BY THE USER. THE      %
% /      \|__/ \__ |    |__/    SNAPSHOTS ARE SELECTED BY THE USER AND THE     %
%                               RESIDUAL ON THE CONTINUITY EQUATION IS         %
% MEASURED PER EACH MODE                                                       %
%                                                                              %
% * * * CALLS * * *                                                            %
%               i.abCFD_                                                       %
%              ii.abCFD_                                                       %
%  <>    ><    <>    ><    <>    ><    <>    ><    <>    ><    <>    ><    <>  %
% abCFD_ReducedOrderModel( var, mesh, REF, SOL, Ee, model, ph, {varargin} )
% INPUTS:
%   - var       :   common structure
%   - mesh      :   common mesh structure
%   - REF       :   structure containing the reference field and its information
%   - SOL       :   structure containing the solution field and its information
%   - Ee        :   Array with minium and maximum number of modes to be used
%   - model     :   common COMSOL's structure
%   - ph        :   common phase structure
%   - varargin  :   eventually specify how to take snapshots


function abCFD_ReducedOrderModel( var, mesh, REF, SOL, Ee, model, ph, varargin)
...
% Assemble matrices Dpp, Dnn, Dnp equal to D
...
% In this section the snapshots are collected from the known solution. If
% not specified in the inputs, the user select manully the snapshosts
...
% Define x-y VELOCITY and TEMPERATURE Probes points at the outlet
...
% Plot nodes' tag for three probes elements
subplot( 2 , 3, [ 1 2 4 5 ] )   
hold all
...
%% Set initial and final time for capturing snapshots
...
if numel(varargin) == 0
    ...
    % This applies if the interval was specified in the inputs
    InitialTime = varargin{1}(1);
    FinalTime =   varargin{1}(2);
end

% Find closest snapshot to the specified value
...
% Print to screen data on the specified interval for snapshot capturing
...
switch SnapMethod
        case 'Double step' % Define two intervals to take snapshots with different time steps
...
% The homogeneous solution isn't stored but is comptued with:
%   bsxfun( @minus, SOL.d1(:,PodInvervals(1) : 2 : PodInvervals(2) )
% The modes are derived from the snapshots collected earlier

% Number of modes
M = numel( SnapShots );
tiin = toc;
disp( char('Specify the method to compute the modes:',...
                       ' 1) eigORT ',...
                       ' 2) eigUTU ',...
                       ' 3) eigsORT',...
                       ' 4) eigsUTU',...
...
switch ModesFun
    case { 'eigORT' , '1' }
%   This choice guarantees orthogonality for the modes and therefore it
%   follow the spirit of POD. However computing the eigenvalue problem with
%   Matlab would lead to immaginary modes (due to asymmetric matrix..weird
%   crap done my Matlab..). The solution of this problem is to take only
%   the symmetric part of that snapshot matrix. This operation, which is
%   linear and does not affect the modes, provides a matrix which is
%   symmetric and suitable for the eigenvalue decomposition.
...
if abCFD_round( sum(diag( B' * B ) ), 1e-1 ) ~= eeMax
...
%
%       MODIFY THE CODE STARTING FROM HERE QUICKLY CODE THE FOLLOWING:
% 
%           1 - RECONSTRUCT PHYSICAL SOLUTION - not all of the three
%               solutions, just one from Matlab's solver but for all the number
%               of modes.
%           2 - ANALYZE THE ACCURACY OF THE RESULTS - Fist analyze the
%               dynamic reduced order system, here compare all the three
%               solutions. Then inspect the physical quantities as function
%               of time in different fixed points.
%           3 - COMPARE THERMAL AND KINETIC ENERGIES - plot these for a
%               solution from Matlab's solver in time for different number of
%               modes.
%           4 - EVENTUALLY PLOT THINGS NOT DEFINED ABOVE - things like
%               solution (physical and reduced order dynamic system), mesh,
%               etc must be available for plot under request of the user
% 

% * + * + * + * + * + * + * + * + * + * + * + * + * + * + * + * + * + * + * + *%

% 
%       HIGHLY DESIRED FEATURE TO BE IMPLEMENTED IS THE SEGREGATED SOLVER
%       IN WHICH: 
%           1.  A full order model is computed until velocity field reaches
%               quasi-steady regime
%           2.  Then with a SEGREGATED solver the velocity and temperature 
%               fields are computed via ROM and FOM, respectively.
%           3.  It will be necessary to compute the residual on the
%               governing equations, therefore the Poisson equation must be
%               solved to compute the pressure given a certain velocity
%               field. For this look at the existing Matlab FEM code
...
% ---                                                                      --- %
%   Version: 0.1                                 date:  SEPTEMBER 2013         % 
% ---                                                                      --- %
%   revision hystory:                                               -  date -  %
%   0.1 - The new version of this routine is working and much more  13/09/2013 %
%         ordered. Even though has not been completed yet, the new steps to be %
%         taken are well defined.
%   0.0 - kick off. This first version was used to produce the      26/06/2013 %
%         results published for the Master thesis. Basically the whole routine %
%         had to be rewritte in order to make it clearer and straight          %
%         forward to understand. A second goal of the rewriting was to order   %
%         the routine as it should be soon be implemented a segregated scheme  %
% ---------------------------------------------------------------------------- %
